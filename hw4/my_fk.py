...
def your_fk(DH_params : dict, q, base_pos) -> np.ndarray:
    ...
    # 儲存每個關節在 world frame 中的 z 軸方向與原點位置，供 Jacobian 使用
    z_axes  = []
    origins = []
    ...
    for i, dh in enumerate(DH_params):
        ...
        # 在套用 T_i 之前，A 代表 base → frame_{i-1}
        # 記錄關節 i 的原點與旋轉軸（z 軸）在 world frame 中的方向
        origins.append(A[:3, 3].copy())   # p_i
        z_axes.append(A[:3, 2].copy())    # z_i
    ...
    # 建立幾何 Jacobian (6×6)
    # 對 revolute joint：Jv_i = z_i × (p_end - p_i)，Jw_i = z_i
    for i in range(6):
        Jv = cross(z_axes[i], p_end - origins[i])
        Jw = z_axes[i]
        jacobian[:3, i] = Jv
        jacobian[3:, i] = Jw
